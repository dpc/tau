...
reviewed commit when building an exact release candidate.
...
Rust **1.97.0** archives, and nFPM **2.46.3** archives.
...
## Build isolation and static packaging integrity
...
Cargo fetching is network-enabled. Notice generation and assembly are
network-disabled. Assembly reads back every archive to compare its exact contents,
inventory, modes and ownership against the staged payload. It does not execute
the delivered binaries. Core build identity is stamped into fixed-size slots;
source manifests, static ELF inspection and checksums describe the resulting packages.

The release build does not repeat application tests or perform help, Hello,
supervisor, service, or distro install/remove qualification. It needs no nested
supervisor namespaces, extra capabilities, or custom seccomp/AppArmor policy.
...
manifests, build/notice/assembly logs, and `SHA256SUMS`.
The build manifest records source/workflow identities, pins, image IDs, per-asset
hashes and upstream inventory. It explicitly records runtime qualification as
`not-performed`; no removed runtime gate is marked passed.
...
static ELF/license metadata. No shell glob decides what is required. The publisher
...
Source readiness is not a completed hosted build. Review and mandatory local SelfCI
...
newer-master race must not silently build different tooling. Both hosted native
...
not application execution evidence. These packaging checks do not establish
supervisor, provider, shell/PTY/CA, kernel, or every distro/CPU/libgcc behavior.
...
The cargo-about registry checksum was resolved on September 14, 2026.
Committed digests, not mutable tags, control selection.
