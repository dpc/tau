...
pub enum OAuthErrorKind {
    ...
    InvalidResponse,
    /// The OAuth server rejected an otherwise successful protocol exchange.
    Authorization,
    /// A time-bounded OAuth flow expired before authorization completed.
    TimedOut,
}
...
impl OAuthError {
    ...
    pub(super) fn authorization(provider_code: &str, message: Option<&str>) -> Self {
        Self {
            kind: OAuthErrorKind::Authorization,
            http_status: None,
            provider_code: bounded_oauth_text(provider_code, MAX_OAUTH_ERROR_CODE_CHARS),
            message: message
                .and_then(|value| bounded_oauth_text(value, MAX_OAUTH_ERROR_MESSAGE_CHARS)),
        }
    }

    pub(super) fn timed_out(message: &'static str) -> Self {
        Self {
            kind: OAuthErrorKind::TimedOut,
            http_status: None,
            provider_code: None,
    ...
    fn safe_provider_code(&self) -> Option<&'static str> {
        match self.provider_code() {
            ...
            Some("expired_token") => Some("expired_token"),
...
impl fmt::Display for OAuthError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self.kind {
            ...
            OAuthErrorKind::Authorization => {
                formatter.write_str("OAuth authorization was rejected")?;
            }
            OAuthErrorKind::TimedOut => formatter.write_str("OAuth authorization timed out")?,
        }
