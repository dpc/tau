# ARCH-tau-cli-picker: tau-cli-picker architecture
...
The picker has no network, persistence, subprocess, secret-storage, or model-facing policy boundary. Prompts and labels are display text rather than trusted terminal control data: rendering goes through `tau-term-screen` styled-cell output and bounded truncation. It returns only the selected index and does not log prompts, labels, or input; callers should not put secrets in visible labels. A guard retains best-effort raw-mode restoration during unwinding, while normal returns use the explicit error-reporting cleanup contract above.
