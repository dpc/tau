...
Live byte stats are harness-owned `agent.turn_stats_updated` events, not
built-in provider metadata. They exclude raw wire bytes and tool execution
outputs/results, and they are not transcript or final-response content.
