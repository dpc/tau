...
Every successful result is an ordinary tool-result string enclosed in `<tau_web_content adapter="exa|parallel" operation="search|fetch" content_trust="external">…</tau_web_content>`. Provider text is XML-escaped and unsafe structural Unicode is made visible. Titles, URLs, ranks, sources, and all other provider-returned metadata remain untrusted body claims. `adapter` identifies only Tau's configured adaptation path; it does not authenticate page authorship or truth, and the envelope is defense-in-depth rather than a prompt-injection sandbox. The extension caps successful MCP response bodies and decoded provider text, then enforces the 512 KiB limit again on the final escaped and closed XML; oversize projection is a tool error, not truncation. It allows up to eight in-flight web calls; additional calls fail fast with a busy tool error.
