...
- Internal `websearch_parallel_fetch`, model-visible as `web_fetch`, is registered but disabled by default. It fetches/extracts one URL through Parallel.ai. Tau accepts singular `url`, adapts it to Parallel's wire-level `urls: [url]`, and passes other JSON-convertible provider-specific arguments through unchanged except an input `urls`, which the adapter replaces.
