...
- Receive grants only Tau-issued source replies backed by extension-local state from a written incoming fact. Message fact IDs use the documented `slack:<channel>:<message-ts>` form, but native IDs/roots are never accepted as separate model-selected route arguments. Proactive send is a separate current-alias grant and grants no receive, linking, control, edit, inbound-human-reaction routing, or source-reply authority.
