...
Private reply, edit, and reaction state is installed only around successful local
message-fact publication while the Slack installation and session remain
current. Failed publication installs nothing. A bounded process-local native-id
...
- `slack_send` owns a 1,024-entry non-evicting session/process ledger. It
  ...
  publication; disconnect/EOF retires authority before workers are woken.
  A successful remote post writes `message.sent` and then its ordinary
  ...
  Awaiting-publication, completed, definitive, cumulative
...
- Agent-authored text cannot contain raw `<@`, `<!`, or `<#` Slack controls.
  ...
  logical call, and Slack revalidates current authority before publication.
...
- Recheck these invariants and the adversarial matrix in `specs/ARCH-tau-ext-slack.md` when changing config freeze, routing, sender admission, mutations, dedup, lifecycle, or message-fact publication.
