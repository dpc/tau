...
and from dynamic DM links. `slack_send` accepts `message` plus exactly one Tau-issued
...
The harness authorizes the tool invocation; the extension revalidates its
connection, session, agent lifecycle, alias, endpoint, kind/id, and fixed thread. Delivery follows
...
same-session replay ledger and cancellation before stale retry or publication.
