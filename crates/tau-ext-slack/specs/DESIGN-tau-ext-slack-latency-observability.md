...
bounded event/outcome/depth classes, process-local connection/occurrence ordinals,
and extension- or harness-owned process-local request correlation. It never logs
...
The stages are websocket frame receipt, envelope decode, pre-ACK reservation, ACK
attempt/completion, FIFO dequeue, identity start/completion, post start/completion,
extension submit/result, and harness activation/commit. “ACK written” means local
websocket flush only. Queue wait starts after successful ACK handoff. Harness
correlation retains only the validated numeric suffix of the Slack extension's
process-local request ordinal; arbitrary extension-controlled request data is never
logged.
