...
events are folded. At the Tau Swarm v0 publication boundary, the
...
Tau Swarm v0 has no self-reported Waiting work-status variant. The projection
therefore maps Tau `Waiting { task_name }` lossily to Swarm v0
...
The extension registers `task_info`, `task_blocker`, and `task_update` in the `swarm` tool group
...
`task_info` atomically replaces the title and optional description for one exact
task ID in the current `SessionProjection`. Task IDs have no owner: ordinary Tau
role tool policy is the complete grant boundary, and any loaded agent granted
