...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let [socket, harness_state, config_dir, state_dir, status] = args.as_slice() else {
        return Err("expected SOCKET HARNESS_STATE CONFIG_DIR STATE_DIR {new|resumed}".into());
    };
    ...
    tau_harness::run_daemon(
        ...
        tau_harness::ServeOptions::builder()
            ...
            .allowed_extensions(BTreeSet::from(["e2e-fake-provider".into()]))
            .build(),
    )?;
    Ok(())
}
