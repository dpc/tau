...
Metadata and body fetches are bounded. Header fetches use a fixed byte window, body reads have a byte and line cap, and outputs mark truncation. `email_list_recent(days=N)` captures one current time and includes IMAP internal timestamps between its inclusive rolling `N × 24 hour` cutoff and that captured time. IMAP `SINCE` accepts calendar dates, so Tau searches from one conservative UTC calendar day before the cutoff, then exact-filters fetched internal timestamps before applying a page limit or cursor. This can fetch an extra calendar day from servers, but it avoids losing qualifying mail at UTC/server calendar boundaries. Tau fails visibly if bounded metadata-fetch or candidate-row budgets exhaust before it fills a filtered page, or if a fetched internal timestamp is missing or malformed; it never silently omits those candidates. `email_id` and folder arguments are validated before use, and returned UIDs are checked against the requested message.
