//! One-pass semantic decoding with exact borrowed raw JSON projections.
...
pub(super) struct DecodedEvent<'a> {
    ...
    /// Exact terminal `output` value, when uniquely present.
...
impl<'a> DecodedEvent<'a> {
    ...
    pub(super) fn decode(raw: &'a str) -> Result<Self, serde_json::Error> {
        ...
        Self::from_value(raw, value)
    }

    /// Decode only the semantic event so transport-owned error classification
    /// can retain its historical precedence over sidecar validation.
    pub(super) fn decode_value(raw: &str) -> Result<Value, serde_json::Error> {
        #[cfg(test)]
        SEMANTIC_DECODES.set(SEMANTIC_DECODES.get() + 1);
        serde_json::from_str(raw)
    }

    /// Index exact sidecars around one semantic value decoded from `raw`.
    pub(super) fn from_value(raw: &'a str, value: Value) -> Result<Self, serde_json::Error> {
        #[cfg(test)]
        RAW_INDEX_SCANS.set(RAW_INDEX_SCANS.get() + 1);
        let mut top = object_members(raw, &["item", "response", "output"])
            .map_err(raw_index_error)?
            .into_iter();
        let item = top.next().flatten();
        let response = top.next().flatten();
        let top_output = top.next().flatten();
        let nested_output = response
            .as_ref()
            .filter(|range| raw[(*range).clone()].trim() != "null")
            .map(|range| object_members(&raw[range.clone()], &["output"]))
            .transpose()
            .map_err(raw_index_error)?
            .and_then(|mut members| members.pop().flatten())
            .map(|range| {
                let base = response.as_ref().expect("nested output has response").start;
        ...
        Ok(Self {
            ...
            raw,
            item,
            output: nested_output.or(top_output),
        })
    }
    ...
    /// Return exact elements from the terminal output array.
    pub(super) fn raw_output_items(&self) -> Result<Option<Vec<&'a str>>, ()> {
        self.output
            .as_ref()
            .map(|range| {
                array_elements(&self.raw[range.clone()]).map(|elements| {
                    elements
                        .into_iter()
                        .map(|element| {
                            let base = range.start;
                            &self.raw[(base + element.start)..(base + element.end)]
                        })
                        .collect()
                })
            })
...
fn raw_index_error(_: RawIndexError) -> serde_json::Error {
    <serde_json::Error as serde::de::Error>::custom("invalid raw event index")
}

fn object_members(raw: &str, targets: &[&str]) -> Result<Vec<Option<Range<usize>>>, RawIndexError> {
    ...
    loop {
        ...
        for (target, found) in targets.iter().zip(&mut found) {
...
fn array_elements(raw: &str) -> Result<Vec<Range<usize>>, ()> {
    let bytes = raw.as_bytes();
    let mut cursor = skip_ws(bytes, 0);
    if bytes.get(cursor) != Some(&b'[') {
        return Err(());
    ...
    loop {
        ...
        let start = cursor;
        let end = value_end(bytes, cursor, 0).map_err(|_| ())?;
