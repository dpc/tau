...
use tau_proto::{
    ...
    MessageEndpoint, MessageEnvelope, MessageId, MessageOperation, MessageReplyPath,
    MessageTransportRef, MessageTrust, RegisterTransportCapabilityRequest,
    RegisterTransportCapabilityResult, ReplyPathLifetime, ReplySelector, SenderPolicyStatus,
    ToolName, TransportMessageDraft, TransportMessageIngressOutcome,
    TransportMessageIngressRequest, TransportMessageIngressResult, TransportSendAuthorization,
};

use super::{
    AgentMessageRecipientStatus, CompletedTransportSend, Harness, PendingIngressAck,
    PendingTransportSendAck, TransportCapability, TransportOrderingRouteKey, TransportReplyRoute,
};
...
impl Harness {
        ...
        match event {
            Event::AgentMessageIncoming(message) => {
                if let (Some(extension_name), Some(_)) = (
                    ...
                    let route_key =
                        ordering_route_key_from_envelope(extension_name, &message.envelope);
                    if let Some(pending) =
    ...
    fn begin_transport_message_ingress(
        ...
        self.pending_ingress_acks.insert(
            message_id,
            PendingIngressAck {
    ...
    pub(super) fn complete_ingress_commit(&mut self, message: &AgentMessageIncoming) {
        ...
        let live_commit = waiter.session_generation == self.current_session_generation
                ...
                .is_some_and(|capabilities| {
                    capabilities.iter().any(|capability| {
                        capability.session_generation == self.current_session_generation
                            ...
                            && capability.send_tool.as_ref() == Some(&send_tool)
                    })
                })
        {
...
mod tests {
    ...
    /// Every authorized ingress request is independently appended and
    /// acknowledged; adapters own any transport-specific duplicate filtering.
    #[test]
    fn harness_ingress_rpc_appends_each_submission() {
        ...
        let tree = harness
            ...
            .expect("target tree");
        assert_eq!(tree.all_message_envelopes().count(), 2);
    }
    ...
    fn draft(event_id: &str) -> TransportMessageDraft {
    ...
    fn incoming_message(
        ...
        ordering: Option<tau_proto::MessageOrdering>,
    ) -> AgentMessageIncoming {
        AgentMessageIncoming {
            ...
            envelope: MessageEnvelope {
                ...
                transport: MessageTransportRef {
                    name: "slack".to_owned(),
                    instance: Some(ExtensionName::new("std-slack")),
                },
                ...
                ordering,
                occurred_at: None,
                reply_path: send_tool.map(|tool_name| MessageReplyPath {
                    tool_name,
                    selector: ReplySelector::ReplyToMessage,
                    lifetime: ReplyPathLifetime::ActiveSession,
                }),
    ...
    /// A committed incoming fact wakes its live recipient even if the extension
    /// waiter disconnected or was cleared after the append began.
    #[test]
    fn committed_ingress_without_waiter_still_delivers() {
        let temp = tempfile::TempDir::new().expect("tempdir");
        let mut harness =
            super::super::tests::echo_harness(temp.path()).expect("test harness starts");
        harness
        ...
        let agent_id = harness
            ...
            .map(crate::parse_agent_id)
            .expect("durable target");
        let message = incoming_message(agent_id.clone(), "committed-without-waiter", None, None);

        harness.publish_for_agent(&cid, Event::AgentMessageIncoming(message));
        ...
        assert!(
            harness.agents[&cid]
                ...
                || harness.event_log.entries_for_test().iter().any(|entry| {
                    matches!(
                        &entry.event,
                        Event::AgentPromptStarted(started) if started.agent_id == agent_id
                    )
                }),
            "delivery must remain queued or start a recipient prompt"
        );
    }

    /// Capability replacement after an append has begun cannot resurrect the
    /// reply route captured by the older registration.
    #[test]
    fn capability_refresh_before_commit_prevents_route_installation() {
        let temp = tempfile::TempDir::new().expect("tempdir");
        let mut harness =
            super::super::tests::echo_harness(temp.path()).expect("test harness starts");
        let frames = super::super::tests::connect_test_tool(&mut harness, "transport-test");
        let message_id = MessageId::new("capability-refresh");
        harness.pending_ingress_acks.insert(
            message_id.clone(),
            PendingIngressAck {
                connection_id: "transport-test".to_owned(),
        ...
        let message = incoming_message(
            tau_proto::AgentId::parse("agent-a").expect("agent"),
            message_id.as_str(),
            Some(ToolName::new("slack_send")),
            None,
        );

        harness.complete_ingress_commit(&message);

        assert!(!harness.transport_reply_routes.contains_key(&message_id));
        assert!(frames.lock().expect("frames").iter().any(|frame| matches!(
            &frame.frame,
            HarnessOutputMessage::TransportMessageIngressResult(result)
                if result.request_id == "request-1"
                    && result.outcome == Some(TransportMessageIngressOutcome::Accepted)
        )));
    }

    /// A failed durable append releases its source-sequence reservation so a
    /// later retry does not inherit an uncommitted ordering conflict.
    #[test]
    fn failed_ingress_commit_releases_ordering_reservation() {
        let temp = tempfile::TempDir::new().expect("tempdir");
        let mut harness =
            super::super::tests::echo_harness(temp.path()).expect("test harness starts");
        let ordering = tau_proto::MessageOrdering { source_sequence: 7 };
        let message = incoming_message(
            tau_proto::AgentId::parse("agent-a").expect("agent"),
            "failed-ordered",
            None,
        ...
        let route_key = ordering_route_key_from_envelope(
            message
                .envelope
                .transport
                .instance
        ...
        harness.fail_transport_publish(&Event::AgentMessageIncoming(message));
    ...
    /// Alias provenance and transport-identity mention constraints remain
    /// enforced independently of duplicate policy.
    #[test]
    fn ingress_validates_identity_alias_and_transport_mention() {
        let mut invalid_alias = draft("event-1");
        ...
        let mut invalid_mention = draft("event-2");
        invalid_mention.transport_identity_mentioned = true;
        invalid_mention.operation = MessageOperation::Delete {
            target: tau_proto::MessageRef::default(),
        };
        assert_eq!(
            validate_draft(&invalid_mention),
