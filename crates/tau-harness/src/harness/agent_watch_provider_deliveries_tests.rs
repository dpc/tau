...
fn provider_status_delivery_generations_are_monotonic_and_reset_forward() {
    ...
    assert!(
        deliveries
            .record(
                tau_proto::AgentOuterTurnGeneration::from_raw(7),
                &first_prompt,
                retrying_transport()
            )
            .should_deliver
    ...
    let stale = deliveries.record(
        tau_proto::AgentOuterTurnGeneration::from_raw(6),
    ...
    let newer_prompt = tau_proto::AgentPromptId::parse("sp-generation-newer")
        .expect("known-safe AgentPromptId must be valid");
    let newer = deliveries.record(
        tau_proto::AgentOuterTurnGeneration::from_raw(8),
        &newer_prompt,
        retrying_transport(),
    );
...
fn provider_status_delivery_capacity_is_fifo_and_terminals_do_not_consume_it() {
    ...
    for index in 0..MAX_TRACKED_PROVIDER_STATUS_PROMPTS {
        assert!(
            deliveries
                .record(
                    tau_proto::AgentOuterTurnGeneration::from_raw(3),
...
fn provider_status_delivery_stays_bounded_across_a_long_high_cardinality_turn() {
    ...
    for index in 0..HIGH_CARDINALITY_PROMPTS {
        ...
        assert!(
            deliveries
                .record(
                    tau_proto::AgentOuterTurnGeneration::from_raw(11),
                    &prompt,
                    retrying_transport()
                )
                .should_deliver
