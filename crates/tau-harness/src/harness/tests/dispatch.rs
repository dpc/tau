...
fn replay_respects_activation_checkpoint_ranges_and_uncertainty() {
    ...
    let completed_prompt_id = tau_proto::AgentPromptId::from("ap-main-completed");
    append_seed_agent_event(
        &mut store,
...
fn standalone_compaction_retry_preserves_owed_and_later_activations() {
    ...
    let first_compact = read_nth_prompt_created(&h, 0);
...
/// A long tool turn can produce many distinct provider prompt ids.
/// Terminal-error prompts must not accumulate in per-subscription dedupe state,
/// while a malformed flood of nonterminal prompts remains capped without
/// weakening first, category-change, duplicate, or terminal delivery behavior.
#[test]
fn agent_watch_provider_dedupe_stays_bounded_during_high_cardinality_long_turn() {
    let td = TempDir::new().expect("tempdir");
    let mut h = echo_harness(td.path().join("state")).expect("start");
    let watched_cid = ensure_test_user_agent(&mut h);
    let watcher_cid =
    ...
    let watcher_id = durable_agent_id_for_conversation(&h, &watcher_cid).to_string();
    h.agents.get_mut(&watcher_cid).expect("watcher").turn_state = AgentTurnState::AgentThinking {
        agent_prompt_id: "high-cardinality-watcher".into(),
    };
    h.set_agent_watch(
        &watcher_id,
        &watched_id,
    ...
    let session_id = h.current_session_id.clone();
    let generation = 17;
    let status = |prompt: String, state| tau_proto::AgentWatchProviderStatusNotification {
        session_id: session_id.clone(),
        subscription_id: String::new(),
    ...
    let status_event_count = |h: &Harness| {
        session_agent_message_received_events(h)
            .into_iter()
            .filter(|message| {
                message.kind == tau_proto::AgentMessageKind::WatchProviderStatus
    ...
    let before = status_event_count(&h);
    let prompt_total =
        crate::harness::agent_watch_provider_deliveries::MAX_TRACKED_PROVIDER_STATUS_PROMPTS * 4;

    for index in 0..prompt_total {
        let prompt = format!("sp-terminal-error-{index}");
        let transport = tau_proto::AgentWatchProviderState::Retrying {
            category: tau_proto::AgentWatchProviderCategory::Transport,
            attempt: 1,
        ...
        h.update_agent_watch_provider_status(
            &watched_id,
            status(prompt.clone(), transport.clone()),
        );
        h.update_agent_watch_provider_status(&watched_id, status(prompt.clone(), transport));
        h.update_agent_watch_provider_status(
            &watched_id,
            status(
                prompt.clone(),
        ...
        let deliveries = &h.agent_watch_provider_deliveries[&subscription_id];
        assert_eq!(deliveries.prompt_count(), 0);
        assert_eq!(deliveries.delivery_key_count(), 0);
    }
    assert_eq!(
        status_event_count(&h) - before,
        prompt_total * 3,
        "each terminal-error prompt delivers first category, category change, and terminal once"
    );
    ...
    assert_eq!(
        ...
        crate::harness::agent_watch_provider_deliveries::MAX_TRACKED_PROVIDER_STATUS_PROMPTS
    );
    assert_eq!(
    ...
    let before_untracked_terminal = status_event_count(&h);
    h.update_agent_watch_provider_status(
        &watched_id,
        status(
            "sp-untracked-terminal".to_owned(),
            tau_proto::AgentWatchProviderState::TerminalError {
                failure_kind: tau_proto::ProviderFailureKind::RequestRejected,
                attempt: 1,
            },
    ...
    let oldest_retained_prompt = format!(
        "sp-nonterminal-{}",
        prompt_total
            - crate::harness::agent_watch_provider_deliveries::MAX_TRACKED_PROVIDER_STATUS_PROMPTS
    );
    ...
    assert_eq!(
        status_event_count(&h) - before_untracked_terminal,
        1,
        "an untracked terminal delivers without evicting a retained prompt's duplicate key"
    );
    let deliveries = &h.agent_watch_provider_deliveries[&subscription_id];
    assert_eq!(
        deliveries.prompt_count(),
    ...
    let newest_prompt = format!("sp-nonterminal-{}", prompt_total - 1);
    let before_newest_transitions = status_event_count(&h);
    h.update_agent_watch_provider_status(
        &watched_id,
        status(
            newest_prompt.clone(),
    ...
    assert_eq!(
        status_event_count(&h) - before_newest_transitions,
        2,
        "same-category retry stays suppressed, then category and terminal deliver"
    );
    let deliveries = &h.agent_watch_provider_deliveries[&subscription_id];
    assert_eq!(
        deliveries.prompt_count(),
        crate::harness::agent_watch_provider_deliveries::MAX_TRACKED_PROVIDER_STATUS_PROMPTS - 1
    );
    assert_eq!(
        deliveries.delivery_key_count(),
        crate::harness::agent_watch_provider_deliveries::MAX_TRACKED_PROVIDER_STATUS_PROMPTS - 1
...
fn unloading_agent_watcher_retires_topology_and_stops_durable_fanout() {
    ...
    assert!(!h.agent_watch_provider_status.contains_key(&watcher_id));
    assert!(
        h.agent_watch_provider_deliveries
            .keys()
            .all(|subscription| h
...
fn unloading_watched_agent_clears_status_and_stops_durable_fanout() {
    ...
    assert!(h.watchers_for_agent(&watched_id).is_empty());
    assert!(!h.agent_watch_provider_status.contains_key(&watched_id));
...
fn terminating_agent_route_rejects_direct_work() {
    ...
    let mut h = echo_harness(td.path().join("state")).expect("start");
    let cid = ensure_test_user_agent(&mut h);
    let recipient_id = h.agents[&cid].agent_id.clone().expect("durable agent");
