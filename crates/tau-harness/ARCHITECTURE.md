...
Provider retry visibility is harness-owned. After validating provider source and prompt ownership, the harness stores one sanitized current snapshot and fans it out only to current session-local watchers. Delivery is deduplicated by subscription, turn generation, prompt, phase, and closed category while that prompt is retained: repeated same-category attempts update the late-watch snapshot without waking the model. Per-subscription bookkeeping retires a prompt after terminal-error delivery and retains at most 64 nonterminal prompt identities within one generation. The cap is far above ordinary serial provider work while imposing a fixed bound on malformed or unusually long tool turns; an evicted prompt is treated as new if it appears again. Trace fields expose retained prompt/key cardinality, and capacity eviction emits a debug event. First/category-change/terminal notifications are durable recipient facts; replay folds them but never re-fans them out. Enabling or re-enabling emits a non-prompt initial client snapshot and includes current state in the tool result. Disable, prune, and session shutdown remove applicable runtime state.
