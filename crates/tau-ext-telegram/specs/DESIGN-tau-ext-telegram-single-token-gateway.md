# DESIGN-tau-ext-telegram-single-token-gateway: Multi-session Telegram uses a single-token gateway
...
Telegram sends. Sidecars retain only their harness-local registration and
prompt-submission responsibilities and never poll Telegram or select raw chat ids.
...
while leaving each harness responsible for validating loaded agents and recording normal
prompt facts; gateway traffic does not masquerade as agent-to-agent messaging.
...
advancing the offset but before a sidecar drains its live queue can therefore lose that
prompt; the design does not claim a durable delivery acknowledgement. The socket and
...
The current component, socket protocol, routing, lifecycle, and persisted state are
described by [ARCH-tau-telegram-gateway](ARCH-tau-telegram-gateway.md).
