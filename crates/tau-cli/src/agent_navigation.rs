...
pub(crate) struct AgentNavigation {
    ...
    live_agents: HashSet<tau_proto::AgentId>,
    /// Modes received in complete harness-authored operational snapshots.
    modes: HashMap<tau_proto::AgentId, tau_proto::AgentNavigationMode>,
    /// Latest authoritative outer-turn runtime state for each loaded agent.
    runtime_states: HashMap<tau_proto::AgentId, tau_proto::AgentRuntimeState>,
}

impl AgentNavigation {
    /// Record a loaded agent without changing an existing navigation mode.
    pub(crate) fn mark_live(&mut self, agent_id: tau_proto::AgentId) {
    ...
    pub(crate) fn apply_stats(
        ...
        if self.live_agents.contains(agent_id) {
            self.modes.insert(agent_id.clone(), navigation_mode);
            self.runtime_states.insert(agent_id.clone(), runtime_state);
        }
    }

    /// Remove every navigation fact for an unloaded endpoint.
    pub(crate) fn unload(&mut self, agent_id: &tau_proto::AgentId) {
    ...
    pub(crate) fn is_active(&self, agent_id: &tau_proto::AgentId) -> bool {
    ...
    pub(crate) fn active_agents(&self) -> HashSet<tau_proto::AgentId> {
        self.live_agents
            .iter()
    ...
    pub(crate) fn is_live(&self, agent_id: &tau_proto::AgentId) -> bool {
        self.live_agents.contains(agent_id)
    }

    /// Return all currently loaded agent ids.
    pub(crate) fn live_agents(&self) -> HashSet<tau_proto::AgentId> {
