...
enum FixtureTransport {
    /// Responses HTTP server-sent events.
    HttpSse,
...
fn curated_provider_vcr_replay_only_lane() {
    ...
    for declaration in manifest.cassettes {
        ...
        let transport = match declaration.transport {
            FixtureTransport::HttpSse => tau_proto::ProviderBackendTransport::HttpSse,
            FixtureTransport::Websocket => tau_proto::ProviderBackendTransport::Websocket,
        };
        assert_eq!(
        ...
        let state = match declaration.transport {
            FixtureTransport::HttpSse => responses_stream_replay(&cassette.stream, &mut |_| {}),
            FixtureTransport::Websocket => ws::run_replay(&cassette.stream, &mut |_| {}),
        }
