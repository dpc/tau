# SPEC-tau-provider-chatgpt-streaming-replay: Streaming and replay
...
The sampler starts when the backend request is dispatched. Lower-layer HTTP/SSE reads
and WebSocket frame receives immediately advance the prompt-local received byte counter
before semantic parsing, while parsed chunks update pending visible/non-visible deltas.
