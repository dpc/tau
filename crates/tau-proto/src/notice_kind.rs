//! Stable first-party `harness.notice.kind` values.

pub const EXTENSION_CONFIG_ERROR: &str = "extension.config_error";
pub const EXTENSION_OPTIONAL_SKIPPED: &str = "extension.optional_skipped";
pub const EXTENSION_STATE_ACCESS: &str = "extension.state_access";
pub const EXTENSION_NOTICE: &str = "extension.notice";
pub const HARNESS_CONFIG_ERROR: &str = "harness.config_error";
pub const HARNESS_FAILURE: &str = "harness.failure";
pub const HARNESS_INTERNAL_WARNING: &str = "harness.internal_warning";
pub const HARNESS_NOTICE: &str = "harness.notice";
pub const HARNESS_REPLAY_ERROR: &str = "harness.replay_error";
pub const HARNESS_INTRODUCTION: &str = "harness.introduction";
pub const HARNESS_SHELL_SNAPSHOT_OMITTED: &str = "harness.shell_snapshot_omitted";
pub const MODEL_SELECTION: &str = "model.selection";
pub const SKILL_COLLISION: &str = "skill.collision";
pub const UI_COMMAND_ERROR: &str = "ui.command_error";
