...
Transient retry updates may carry `ProviderRetryStatus`: a closed work category, saturating attempt number, and approximate whole-second delay. Human status text remains UI presentation and is not an authority for harness decisions. `AgentWatchProviderStatusNotification` is the harness-authored cross-agent projection and contains only bounded facts, prompt/turn correlation, watch subscription identity, and a nested serde-tagged `state`. The `phase` discriminator selects `retrying`, `recovering_context`, `blocked`, `dispatch_uncertain`, `terminal_error`, or `terminal_incomplete`; each variant owns exactly the category, attempt, delay, or failure fields valid for that phase, so contradictory option combinations are neither constructible nor decodable. `recovering_context` represents active harness-authorized reactive compaction. `terminal_incomplete` represents a durable output-length terminal and carries the exact nonzero finite transport attempt authored on `provider.response_finished`. A cold late subscriber receives the same initial sticky snapshot without replaying the terminal as new live activity. Retrying, recovery, blocked-compaction, restored dispatch-uncertain, terminal-error, and terminal-incomplete projections follow the current durable work state.
