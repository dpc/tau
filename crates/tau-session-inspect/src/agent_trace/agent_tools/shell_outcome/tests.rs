...
fn parser_enforces_approved_coherence() {
    ...
    let parsed = ShellOutcome::from_cbor(
        ShellOutcomeSource::ToolResult,
    ...
    let timeout = ShellOutcome::from_cbor(
        ShellOutcomeSource::ToolResult,
        &map(vec![
    ...
    .expect("timeout");
    assert!(timeout.timed_out.is_some());
    assert!(!timeout.success);

    assert!(
        ShellOutcome::from_cbor(
    ...
    let signal = ShellOutcome::from_cbor(
        ShellOutcomeSource::ToolResult,
        &map(vec![
    ...
    let timeout_with_auxiliaries = ShellOutcome::from_cbor(
        ShellOutcomeSource::ToolResult,
        &map(vec![
    ...
    let unknown = ShellOutcome::from_cbor(
        ShellOutcomeSource::ToolResult,
        &map(vec![
    ...
    assert!(unknown.timed_out.is_some());

    let start_error_with_auxiliaries = ShellOutcome::from_cbor(
        ShellOutcomeSource::ToolErrorDetails,
        &map(vec![
    ...
    assert!(ShellOutcome::from_cbor(ShellOutcomeSource::ToolResult, &legacy_with_false).is_none());
    let error_legacy = map(vec![field("status", CborValue::Integer(0.into()))]);
    assert!(ShellOutcome::from_cbor(ShellOutcomeSource::ToolErrorDetails, &error_legacy).is_none());
    ...
    for malformed in [
        ...
        assert!(
            ShellOutcome::from_cbor(ShellOutcomeSource::ToolResult, &malformed).is_none(),
...
fn duplicate_detection_is_bounded_and_projection_specific() {
    ...
    assert!(
        ShellOutcome::from_cbor(ShellOutcomeSource::ToolResult, &CborValue::Map(large)).is_some()
    );
    ...
    assert!(ShellOutcome::from_cbor(ShellOutcomeSource::ToolResult, &duplicate_status).is_none());
}
...
fn terminal_event_mapping_preserves_source_and_omits_placeholders() {
    ...
    let foreground = ShellOutcome::from_terminal_event(&tool_result(ToolResultKind::Final))
        .expect("foreground result");
    assert!(matches!(foreground.source, ShellOutcomeSource::ToolResult));
    assert_eq!(foreground.exit_code, Some(0));
    assert!(
        ShellOutcome::from_terminal_event(&tool_result(ToolResultKind::BackgroundPlaceholder))
            .is_none()
    );
    ...
    assert!(matches!(
        ShellOutcome::from_terminal_event(&foreground_error)
    ...
    assert!(matches!(
        ShellOutcome::from_terminal_event(&background)
            .expect("background result")
            .source,
        ShellOutcomeSource::ToolResult
    ));

    let background_error = Event::ToolBackgroundError(tau_proto::ToolBackgroundError {
        call_id: "call".into(),
        tool_name: tau_proto::ToolName::new("shell"),
        tool_type: tau_proto::ToolType::Function,
        message: "start".into(),
    ...
    assert!(matches!(
        ShellOutcome::from_terminal_event(&background_error)
