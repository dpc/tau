...
The wait queue preserves FIFO fairness among overlapping requests. A waiter may acquire as soon as no active lock conflicts with its requested directories and no earlier waiter overlaps those directories, so a blocked request for one repository does not stop unrelated repositories from proceeding. Same-owner automatic reentry is allowed so an agent holding a manual lock can keep using mutating tools under that lock without deadlocking itself. A repeated manual `dir_lock update` still errors.

Manual locks track acquisition time, last-use time, and active automatic tools running under the lock. A waiter that is blocked by an active conflicting manual lock and not by an earlier overlapping waiter performs a liveness check every 60 seconds. If the blocking manual lock has been idle for 120 seconds and has no active automatic tools inside it, the waiter returns `error: dir_lock_abandoned` with structured details headers: `blocking_directory`, `lock_owner_id`, `idle_seconds`, and `held_seconds`, plus a short text payload in `output`.
